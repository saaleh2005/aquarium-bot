            ...
            "messages": [
                {
                    "role": "system",
                    "content": (
                        "You are an aquarium expert. "
                        "Answer in Persian or English depending on user. "
                        "Focus only on: fish, aquatic plants, amphibians, shrimp, snails, aquascaping, water chemistry."
                    )
                },
                {
                    "role": "user",
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if item:
        ...
        reply = (
            f"🐠 {item.get('fa_name','')} / {item.get('en_name','')}\n\n"
            f"🇮🇷 {item.get('fa_desc','')}\n"
            f"🇬🇧 {item.get('en_desc','')}\n\n"
            f"🌡 Temp: {item.get('temp','')}\n"
            f"⚗️ pH: {item.get('ph','')}\n\n"
            f"🧼 Care:\n"
            f"🇮🇷 {item.get('care', {}).get('fa','')}\n"
            f"🇬🇧 {item.get('care', {}).get('en','')}\n"
        )
